...
def read_volume_profile(vp_raw: dict, asof: int) -> dict | None:
    ...
    -> the forming session's reading `{session, poc, vah, val, volume, high, low}`,
    or None if no session has formed yet.
    ...
    return {
        ...
        "high": cur["high"],            # session high (SH)
        "low": cur["low"],              # session low  (SL)
    }
